...
export default function GameCard({ name, imageUrl, year, genres, gameId }) {
    ...
    return (
        ...
        '>
            <Link to={`/game/${gameId}`}>
                <h1 className='mb-3.5 text-xl font-bold uppercase text-white'>{name} ({year})</h1>
                <div className='flex mx-auto w-fit px-2 sm:px-6 lg:px-0 text-white'>
                ...
                <div className='mx-auto max-w-7xl px-2 sm:px-6 lg:px-8'>
                    {genres.map((genre, index) => {
                        if (index === (genres.length - 1)) {
                            return <span className='mb-2 text-sm font-medium capitalize text-white' key={index}>{genre.name}</span>
                        } else {
                            return <span className='mb-2 text-sm font-medium capitalize text-white' key={index}>{genre.name}, </span>
                        }
                    })}
